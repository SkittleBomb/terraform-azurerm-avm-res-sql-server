...
variable "sqlserver_name" {
  ...
  validation {
    condition     = can(regex("^[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])?$", var.sqlserver_name))
    error_message = "The name must be 1-63 characters long, contain only lowercase letters, numbers, and hyphens, and cannot start or end with a hyphen."
  }
}
...
variable "identity" {
  ...
  validation {
    condition     = try(var.identity == null || contains(["SystemAssigned", "UserAssigned", "SystemAssigned, UserAssigned"], var.identity.type), true)
    error_message = "If identity is defined, type must be one of: SystemAssigned, UserAssigned, SystemAssigned, UserAssigned."
  }
  validation {
    condition     = try(var.identity == null || var.identity.type != "UserAssigned" || length(var.identity.identity_ids) > 0, true)
    error_message = "If identity type is UserAssigned, identity_ids must be defined."
  }
}
...
variable "azuread_administrator" {
  ...
  default = null # Set default to null to make providing this variable truly optional
}
